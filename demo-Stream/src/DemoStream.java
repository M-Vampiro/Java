...
public class DemoStream {
    ...
    public static void main(String[] args) {
        ...
        // fillter out string with length <= 5
        ...
        List<Cat> cats = new ArrayList<>(List.of(Cat.Builder().name("ABC").age(3).build(),
                Cat.Builder().name("DEF").age(5).build(), Cat.Builder().name("IJK").age(7).build()));

        // stream(), name startWith("I"), age > 3

        // filter Cat, and return List<Cat>
        List<Cat> result3 = cats.stream() //
                .filter(e -> e.getName().startsWith("I") || e.getAge() >= 5)
                .collect(Collectors.toList());
        ...
        List<Cat> result1 = sc.filter(e -> e.getAge() >= 10).collect(Collectors.toList());

        Stream<Cat> sc2 = Stream.ofNullable(null); // new ArrayList<>().stream()
        sc2.forEach(e -> System.out.println("hello"));
